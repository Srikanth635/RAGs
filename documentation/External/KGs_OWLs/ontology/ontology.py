...
class OntologyManager(object):
    ...
    def print_ontology_class(ontology_class: Type[Thing]):
        ...
        print(f"Defined class: {ontology_class.get_defined_class()}")
        print(f"Super classes: {ontology_class.is_a}")
        print(f"Equivalent to: {EntityClass.get_equivalent_to(ontology_class)}")
        print(f"Indirectly equivalent to: {ontology_class.get_indirect_equivalent_to()}")
        print(f"Ancestors: {list(ontology_class.ancestors())}")
        print(f"Subclasses: {list(ontology_class.subclasses())}")
        print(f"Disjoint unions: {ontology_class.get_disjoint_unions()}")
        print(f"Properties: {list(ontology_class.get_class_properties())}")
        print(f"Indirect Properties: {list(ontology_class.INDIRECT_get_class_properties())}")
    ...
    def print_ontology_property(ontology_property: Property):
        ...
        print(f"{ontology_property} {property_class}")
        print(f"Relations: {list(ontology_property.get_relations())}")
        print(f"Domain: {ontology_property.get_domain()}")
        print(f"Range: {ontology_property.get_range()}")
        if hasattr(property_class, "_equivalent_to"):
            print(f"Equivalent classes: {EntityClass.get_equivalent_to(property_class)}")
        if hasattr(property_class, "_indirect"):
            print(f"Indirectly equivalent classes: {EntityClass.get_indirect_equivalent_to(property_class)}")
        print(f"Property chain: {ontology_property.get_property_chain()}")
        print(f"Class property type: {ontology_property.get_class_property_type()}")
        print("-------------------")
    ...
    def get_default_ontology_search_path() -> Optional[str]:
        ...
        if onto_path:
            ...
            print("No ontology search path has been configured!")
            return None
    ...
    def create_ontology_world(use_global_default_world: bool = False,
        ...
        if sql_backend_filename and (sql_backend_filename != ONTOLOGY_SQL_IN_MEMORY_BACKEND):
            if not sql_backend_path_absolute:
                print(f"For ontology world accessing, either f{ONTOLOGY_SQL_IN_MEMORY_BACKEND}"
        ...
        try:
            if use_global_default_world:
                ...
                print(f"Using global default ontology world with SQL backend: {sql_backend_name}")
            else:
                # Create a new world with parallelized file parsing enabled
                if sql_backend_path_valid:
                    world = OntologyWorld(filename=sql_backend_filename, exclusive=False,
                                          enable_thread_parallelism=True)
                else:
                    world = OntologyWorld(exclusive=False, enable_thread_parallelism=True)
                print(f"Created a new ontology world with SQL backend: {sql_backend_name}")
        except sqlite3.Error as e:
            print(f"Failed accessing the SQL backend of ontology world: {sql_backend_name}",
    ...
    def load_ontology(self, ontology_iri: str) -> Optional[Tuple[Ontology, Namespace]]:
        ...
        try:
            ...
            if is_local_ontology_iri:
                print(f"Main ontology failed being loaded from {ontology_iri}")
            else:
                print(f"Main ontology failed being downloaded from the remote {ontology_iri}")
            return None
    ...
    def save(self, target_filename: Optional[str] = None, overwrite: bool = False) -> bool:
        ...
        if save_to_same_file and not overwrite:
            ...
            if save_to_same_file and overwrite:
                print(f"Main ontology {self.main_ontology.name} has been overwritten to {save_filename}")
            else:
    ...
    def create_ontology_concept_class(self, class_name: str,
        ...
        if getattr(ontology, class_name, None):
            print(f"Ontology concept class {ontology.name}.{class_name} already exists")
            return None
    ...
    def create_ontology_property_class(self, class_name: str,
        ...
        if getattr(ontology, class_name, None):
            print(f"Ontology property class {ontology.name}.{class_name} already exists")
            return None
    ...
    def create_ontology_triple_classes(self, subject_class_name: str, object_class_name: str,
        ...
        with ontology:
            ...
            if not ontology_subject_class:
                print(f"{ontology.name}: Failed creating ontology subject class named {subject_class_name}")
                return False
            ...
            if not ontology_object_class:
                print(f"{ontology.name}: Failed creating ontology object class named {object_class_name}")
                return False
            ...
            if not ontology_predicate_class:
                print(f"{ontology.name}: Failed creating ontology predicate class named {predicate_class_name}")
                return False
    ...
    def reason(self, world: OntologyWorld = None, use_pellet_reasoner: bool = True) -> bool:
        ...
        try:
            ...
            print(f"{reasoner_name} reasoning failed: {error}")
            return False
        print(f"{reasoner_name} reasoning finishes!")
        return True
